...
const galleryNode: HTMLElement = model.querySelector('#gallery') ?? model.createElement('div');
const sliderNode: HTMLElement = model.querySelector('#slider') ?? model.createElement('div');
...
const albumIterator = function* getAlbumFromAlbumsArrayGenerator (arr: Array<Album>) {
...
const loadOneMoreAlbum = async () => {
    ...
    const album = new Album(`${albumsUrl}/${currentAlbum.id}`, `${usersUrl}/${currentAlbum.userId}`);
    await album.init();
